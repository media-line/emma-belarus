...
class LoadingPeelBackSectorFoxm
{
...
function createImage($in, $out, $w, $h, $transparent){
  ...
  function resize($newW, $newH) {
    ...
    imagecopyresampled($dst_im, $this->res, $newX, $newY, 0, 0, $dst_w, $dst_h, $src_width, $src_height);
        ...
        function cleanSpaceFromFoxm($text) {
            $text = @explode('<arrow_set>', $text); 
            return @$text[1];
        }

        function PasivItemsToFoxm() {
            $db = &JFactory::getDBO();
        ...
        } FoxmArrowsHelper($assetDir);
        
        function isNewOptionInFoxm() {
			return (JVERSION < '1.6.0')? False : True;
		}

class UniversalImageCachingExtenderFoxm {
