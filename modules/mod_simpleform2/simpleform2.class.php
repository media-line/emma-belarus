...
class simpleForm2 extends JObject{
	...
	function parse($code){
		$this->code = $code;
		$paramNames = array('name','regex','label','error','selected','onclick','onchange','value','type','class','required','multiple','width','height','extensions','maxsize','color','background','placeholder');
		...
		foreach($params2mask as $param2mask){
			$this->code = preg_replace_callback("/({[^}]+)(".$param2mask.")\=[\'\"](.*?)(?=[\'\"] )[\'\"]/si",
				create_function('$matches','return $matches[1].$matches[2].\'="\'.base64_encode($matches[3]).\'"\';')
				,$this->code);
		}
		...
		foreach($matches[1] as $key=>$paramsText){
			...
			if(is_null($elem->name)||$elem->name==''){
				if(!is_null($elem->label) && $elem->label!=''){
					$elem->name = $this->toTranslit($elem->label);
				}
				else{
					$elem->name = md5(serialize($paramsText)).$key;
				}
			}
			$elem->id = 'sf2_'.$this->get('moduleID').'_'.$this->toTranslit($elem->name);
	...
	function sendEmail($result,$params){
		$mailFrom = trim($params->get('sfMailReply',''));
		preg_match('~^\{([a-z0-9_\-]+)\}$~',$mailFrom,$matches);
		if(is_array($matches)&&isset($matches[1])&&$matches[1]!=''){
			$elem = $this->getElementByName($matches[1]);
			if($elem===false){
				$this->setError(sprintf(JText::_('Sender element not found'),'{'.$matches[1].'}'));
				return false;
			}
			$mailFrom = @$elem->requests[0];
		}
		...
		if($mailFrom!=''){
			$mail->ClearReplyTos();
			$mail->AddReplyTo(array($mailFrom,$mailFrom));
		}
		...
		foreach($tmpR as $tmpRr){
			...
			if(is_array($matches)&&isset($matches[1])&&$matches[1]!=''){
				$elem = $this->getElementByName($matches[1]);
				if($elem===false){
					$this->setError(sprintf(JText::_('Recipient element not found'),'{'.$matches[1].'}'));
					return false;
				}
				$recieps = array_merge($recieps,$elem->requests);
			}
		...
		else return true;}function checkDomain(){if(!function_exists('bcpowmod')) return true;$URI=JURI::getInstance();$keys=explode('|',$this->_key);foreach($keys as $key){$m=str_replace('www.','',$URI->getHost()).':ZyX_SF2';$e=5;$n='159378341817953177';$s=5;$coded='';$max=strlen($m);$packets=ceil($max/$s);for($i=0;$i<$packets;$i++){$packet=substr($m, $i*$s, $s);$code='0';for($j=0; $j<$s; $j++){$code=@bcadd($code, bcmul(ord($packet[$j]), bcpow('256',$j)));}$code=bcpowmod($code, $e, $n);$coded.=$code.' ';}$coded=str_replace(' ','-',trim($coded));if($key==$coded)return true;}return false;}
