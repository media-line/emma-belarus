...
abstract class FCaptchaDrawer
{
	...
	{
		imagettftext($this->Image['data'], rand($this->Font['min'], $this->Font['max']), rand(-$this->Font['angle'], $this->Font['angle']), rand($position * $space + $this->Font['min'], ($position + 1) * $space - $this->Font['max']), rand($this->Font['max'], $this->Image['height'] - $this->Font['max']), $color, $this->Font['family'], $character);
	}
	...
	private function LoadColor($key, $default)
	{
		...
		if (!$this->validate_hex_color($color))
		{
			$color = $default;
		}
